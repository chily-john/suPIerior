...
- Apply workflow/step `model` and `thinkingLevel` runtime settings through the Pi adapter before sending a start, next, or auto-next step prompt; resolve step settings before workflow defaults and captured garden-start defaults, resolve level names (tiny/small/medium/large/xl) to model references via config or use direct model references, unavailable model candidates warn without blocking the step, and completion restores captured runtime defaults.
