 ...
 */
function saveConfigWithValidation(
  cwd: string,
  config: ModelConfig,
 ...
 */
async function editModelLevels(
  config: ModelConfig,
  ctx: ConfigCommandContext,
): Promise<ModelConfig> {
...
async function editLevelModels(
  ...
  while (!shouldExit) {
    const modelOptions = [...models, "Add model", "Remove a model", "Back to level menu"];
 ...
 */
async function editDefaultModel(
  config: ModelConfig,
  ctx: ConfigCommandContext,
): Promise<ModelConfig> {
  const newConfig = { ...config };
 ...
 */
async function editFallbackStrategy(
  config: ModelConfig,
  ctx: ConfigCommandContext,
): Promise<ModelConfig> {
  const newConfig = { ...config };
  ...
  const newStrategy = await ctx.ui.select("Select fallback strategy:", strategyOptions);
