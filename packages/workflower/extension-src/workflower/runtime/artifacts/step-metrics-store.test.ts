...
import { readConfig, resetConfigCache, ensureMetricsDir, appendStepMetrics } from './step-metrics-store';
...
describe('readConfig', () => {
  ...
  it('should return default config when file has invalid schema', async () => {
    vi.spyOn(fs, 'readFile').mockResolvedValueOnce(JSON.stringify({ metricsEnabled: 'not-a-boolean' }));
  ...
  it('should cache the config after first read', async () => {
    const mockReadFile = vi.spyOn(fs, 'readFile').mockResolvedValue(JSON.stringify({ metricsEnabled: true }));
...
describe('appendStepMetrics', () => {
  ...
  const expectedMetricsDir = resolve('/tmp/test-workflower/.workflower/past-runs-data/test-garden/test-flower');
