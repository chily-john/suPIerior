...
import { applyWorkflowStepRuntimeSettings } from "../extension-src/workflower/package-api/workflow-runtime-settings";
...
describe("model resolution notification", () => {
  ...
  beforeEach(() => {
    ...
    mockResolveModelWithFallbackAndMetadata = vi.fn(
      (level: string | null, config: ModelConfig | null) => {
        if (level === "medium") {
          return {
            result: "openai/gpt-4",
            resolution: {
    ...
    vi.spyOn(modelResolverModule, "resolveModelWithFallback").mockImplementation(
      mockResolveModelWithFallback,
    );
    vi.spyOn(modelResolverModule, "resolveModelWithFallbackAndMetadata").mockImplementation(
      mockResolveModelWithFallbackAndMetadata,
    );
  });
