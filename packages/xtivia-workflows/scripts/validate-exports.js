...
function validateIndexFile(indexFilePath) {
  ...
  try {
    ...
    return {
      isValid: false,
      errors: [`Cannot read index file: ${indexFilePath} - ${error.message}`],
      warnings,
      info,
    };
  }
  ...
  const hasComponentFile =
    existsSync(resolve(dirPath, "Component.tsx")) || existsSync(resolve(dirPath, "Component.jsx"));
  ...
  if (hasComponentFile) {
    ...
    const hasMatchingNamedExport = actualExports.some(
      (e) => getExportedName(e) === expectedComponentName,
    );

    // Also check for default export from Component file
    const hasDefaultExportFromComponent = actualExports.some(
      (e) =>
        e.isDefault &&
    ...
    if (!hasMatchingNamedExport && !hasDefaultExportFromComponent) {
      ...
      errors.push(
        `Component directory '${basename(dirPath)}' should export '${expectedComponentName}' (or default from Component.tsx) but exports: ${exportedNames.join(", ") || "none"}`,
      );
    } else {
      if (hasMatchingNamedExport) {
        ...
        const componentExport = actualExports.find(
          (e) => getExportedName(e) === expectedComponentName,
        );
        if (componentExport) {
          ...
          if (!validSources.includes(componentExport.from)) {
            warnings.push(
              `Component export '${expectedComponentName}' should be from './Component.tsx' but is from '${componentExport.from}'`,
            );
          }
        }
    ...
    for (const sourceFile of sourceFiles) {
      if (!exportedNames.includes(sourceFile)) {
        errors.push(
          `Source file '${sourceFile}.tsx' (or .ts/.jsx) is not exported from ${indexFilePath}`,
        );
      }
    }
  }
  ...
  if (existsSync(partsDir) && statSync(partsDir).isDirectory()) {
    ...
    if (existsSync(partsIndexPath)) {
      ...
      for (const partFile of partsFiles) {
        ...
        if (!hasPartExport) {
          errors.push(
            `parts/index.ts: Expected export '${partBaseName}' from part file '${partFile}' not found`,
          );
        } else {
...
function validatePath(path) {
  ...
  if (existsSync(resolvedPath) && statSync(resolvedPath).isFile()) {
    ...
    return {
      isValid: false,
      errors: [`No index.ts found in directory: ${dirPath}`],
      warnings: [],
      info: [],
    };
  }
  ...
  if (existsSync(resolvedPath) && statSync(resolvedPath).isDirectory()) {
    ...
    return {
      isValid: false,
      errors: [`No index.ts found in directory: ${resolvedPath}`],
      warnings: [],
      info: [],
    };
  }
