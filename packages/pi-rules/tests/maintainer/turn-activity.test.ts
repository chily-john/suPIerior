...
import { addChangedPath, addDeletedPath, addRenamedPath, allActivityPaths, createTurnFileActivity, mergeTurnFileActivity } from "@features/rule-maintenance/turn-activity";
...
describe("TurnFileActivity", () => {
	it("merges activity and deduplicates paths while preserving rename metadata", () => {
		...
		expect(allActivityPaths(target)).toEqual(["src/foo.ts", "src/deleted.ts", "src/old.ts", "src/new.ts"]);
