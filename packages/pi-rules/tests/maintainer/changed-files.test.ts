...
describe("fileActivityFromTool", () => {
  it("detects write and edit tool path inputs as changed paths", () => {
    expect([...fileActivityFromTool(cwd, "write", { path: "src/new.ts" }).changed.keys()]).toEqual([
      "src/new.ts",
    ]);
    expect([
      ...fileActivityFromTool(cwd, "functions.edit", { path: "src/existing.ts" }).changed.keys(),
    ]).toEqual(["src/existing.ts"]);
  });

  it("detects bash removal and rename activity", () => {
    const activity = fileActivityFromTool(cwd, "bash", {
      command: "rm -f src/deleted.ts\nmv src/old.ts src/new.ts",
    });
