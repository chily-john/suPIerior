...
export function protectedRulePathsForBatch(cwd: string, changedFiles: string[]): string[] {
	...
	for (const changedFile of changedFiles) {
		...
		const matches = index.rules.filter((rule) => rule.kind === "rules" && rule.paths.some((pattern) => pathMatchesPatternScope(normalizedChangedFile, pattern)));
	...
	return protectedPaths.size > 0 ? [...protectedPaths].map(normalizeRulePath).sort() : [".pi/rules"];
}
