...
import { addChangedPath, addDeletedPath, addRenamedPath, createTurnFileActivity, type TurnFileActivity } from "@features/rule-maintenance/turn-activity";
...
export function getGitStatusSnapshot(cwd: string): GitStatusSnapshot {
	const result = spawnSync("git", ["status", "--porcelain=v1", "--untracked-files=all"], { cwd, encoding: "utf8", windowsHide: true });
...
function activityKeys(snapshot: GitStatusSnapshot | Set<string>): Set<string> {
	...
	for (const rename of snapshot.activity.renamed.values()) keys.add(`renamed:${rename.fromPath}\u0000${rename.toPath}`);
	return keys;
}

function hasBaselineActivity(start: GitStatusSnapshot | Set<string>, baselineKeys: Set<string>, kind: "changed" | "deleted" | "renamed", filePath: string): boolean {
	if (!(start instanceof Set)) return baselineKeys.has(`${kind}:${filePath}`);
	return start.has(filePath);
}

export function getGitStatusActivitySince(cwd: string, start: GitStatusSnapshot | Set<string>): TurnFileActivity {
	...
	for (const [filePath, metadata] of snapshot.activity.changed) {
		if (!hasBaselineActivity(start, baselineKeys, "changed", filePath)) addChangedPath(activity, filePath, metadata);
	}
	for (const [filePath, metadata] of snapshot.activity.deleted) {
		if (!hasBaselineActivity(start, baselineKeys, "deleted", filePath)) addDeletedPath(activity, filePath, metadata);
	}
	for (const rename of snapshot.activity.renamed.values()) {
		const renameKey = `renamed:${rename.fromPath}\u0000${rename.toPath}`;
		if (!baselineKeys.has(renameKey)) addRenamedPath(activity, rename.fromPath, rename.toPath, rename.metadata);
	}
	return activity;
}
