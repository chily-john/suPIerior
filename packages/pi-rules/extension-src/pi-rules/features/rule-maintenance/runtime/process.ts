...
function resolveJsCliFromShim(shimPath: string): string | undefined {
  ...
  const quotedCandidates = [...content.matchAll(/["']([^"']+\.(?:cjs|js|mjs))["']/gi)].map(
    (match) => match[1],
  );
  const tokenCandidates = [...content.matchAll(/([^"'\s]+\.(?:cjs|js|mjs))/gi)].map(
    (match) => match[1],
  );
  const candidates = [...quotedCandidates, ...tokenCandidates].filter((candidate) =>
    /[\\/]/.test(candidate),
  );
...
export function getPiSpawnSpec(): PiSpawnSpec | undefined {
  ...
  if (cliPath && fs.existsSync(cliPath)) {
    if (/\.(cjs|js|mjs)$/i.test(cliPath))
      return { command: process.execPath, prefixArgs: [cliPath], shell: false };
