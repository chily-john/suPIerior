...
export function parseMaintainerOutputLine(line: string, logPath: string): MaintainerOutputEvent | undefined {
	if (!line.trim()) return undefined;
	try {
		const event = JSON.parse(line) as { type?: string; message?: { role?: string; stopReason?: string; content?: Array<{ type?: string; text?: string }> } };
		if (event.type === "message_end" && event.message?.role === "assistant" && event.message.content) {
			...
			const hasToolCalls = event.message.content.some((part) => part.type === "toolCall" || part.type === "tool_call");
			const stoppedWithError = event.message.stopReason === "error" || event.message.stopReason === "aborted";
