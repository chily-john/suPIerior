...
import type { MaintainerCallbacks, MaintainerPathChangeMetadata } from "@features/rule-maintenance/model";
...
export function runMaintainerBatch(options: RunMaintainerBatchOptions): boolean {
	...
	if (!piSpawn) {
		fs.appendFileSync(logPath, "Maintainer failed to start: could not resolve a non-shell Pi CLI entrypoint. Reinstall or launch Pi through its Node CLI script.\n");
		callbacks.onFinished?.({ ok: false, code: null, error: "could not resolve a non-shell Pi CLI entrypoint", logPath });
		options.onFinalize();
		return false;
	}

	const args = [...piSpawn.prefixArgs, "--print", "--mode", "json", "--no-session", "--no-extensions", "--no-skills", "--skill", maintainerSkillPath(), "--no-context-files", "--tools", "read,edit,bash", buildMaintainerPrompt(changedFiles, reason, options.protectedRulePaths, options)];
	fs.appendFileSync(logPath, `Spawn: ${piSpawn.command} ${piSpawn.prefixArgs.join(" ")} --mode json\n`);
	const child = spawn(piSpawn.command, args, { cwd, stdio: ["ignore", "pipe", "pipe"], windowsHide: true, shell: piSpawn.shell });
	if (child.pid) {
		options.onPid(child.pid);
		fs.appendFileSync(logPath, `Started run ${options.runId} PID: ${child.pid} at ${new Date().toISOString()}\n`);
	}
	callbacks.onStarted?.();
	...
	const killChildOnParentExit = () => {
		...
		try { killProcessTree(child.pid); } catch { /* best-effort session ownership cleanup */ }
		try { fs.appendFileSync(logPath, `Parent Pi process exited; sent termination to maintainer PID ${child.pid} at ${new Date().toISOString()}.\n`); } catch { /* ignore shutdown logging errors */ }
	};
	...
	child.once("spawn", () => fs.appendFileSync(logPath, `Child spawn event at ${new Date().toISOString()}.\n`));
	child.once("error", (error) => finalize(null, error, "error"));
	child.once("exit", (code) => {
		fs.appendFileSync(logPath, `Child exit event at ${new Date().toISOString()} with code ${code ?? "unknown"}.\n`);
		finalize(code, undefined, "exit");
	});
	child.once("close", (code) => {
		if (stdoutBuffer.trim()) handleStdoutLine(stdoutBuffer);
		fs.appendFileSync(logPath, `Child close event at ${new Date().toISOString()} with code ${code ?? "unknown"}.\n`);
