...
import { activeRunAgeMs, isActiveRunSuspicious, readActiveMaintainerRuns } from "@features/rule-maintenance/state/active-runs";
...
export function formatMaintainerStatus(cwd: string, config: HierRulesConfig): string {
	...
	active.forEach((run, index) => {
		...
		lines.push(`  ${index + 1}. ${run.id}: ${alive ? (stale ? "stale/suspicious" : "running") : "stale"}`);
		lines.push(`     PID: ${run.pid ?? "unknown"}; started: ${run.startedAt}; age: ${formatDuration(activeRunAgeMs(run))}`);
	...
	if (legacyLock) {
		...
		if (legacyLock.changedFiles?.length) lines.push(`- Files: ${legacyLock.changedFiles.join(", ")}`);
	}
	...
	pending.slice(0, 5).forEach((batch, index) => {
		...
		if (batch.protectedRulePaths?.length) lines.push(`     Protected rules: ${batch.protectedRulePaths.join(", ")}`);
	});
