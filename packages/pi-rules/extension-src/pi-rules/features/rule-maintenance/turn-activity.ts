...
export function addChangedPath(
...
export function addDeletedPath(
  activity: TurnFileActivity,
  filePath: string,
  metadata?: FileActivityMetadata,
) {
...
export function addRenamedPath(
  activity: TurnFileActivity,
  fromPath: string,
  toPath: string,
  metadata?: FileActivityMetadata,
...
export function mergeTurnFileActivity(target: TurnFileActivity, source: TurnFileActivity) {
  ...
  for (const rename of source.renamed.values())
    addRenamedPath(target, rename.fromPath, rename.toPath, rename.metadata);
}

export function changedActivity(
  activity: TurnFileActivity,
): Array<{ path: string; metadata?: FileActivityMetadata }> {
  return [...activity.changed.entries()].map(([filePath, metadata]) =>
    metadata ? { path: filePath, metadata } : { path: filePath },
  );
}

export function deletedActivity(
  activity: TurnFileActivity,
): Array<{ path: string; metadata?: FileActivityMetadata }> {
  return [...activity.deleted.entries()].map(([filePath, metadata]) =>
    metadata ? { path: filePath, metadata } : { path: filePath },
  );
}
