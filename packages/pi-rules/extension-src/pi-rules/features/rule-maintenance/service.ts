...
import type { MaintainerActiveRun, MaintainerCallbacks, MaintainerPathChangeMetadata, MaintainerQueueBatch } from "@features/rule-maintenance/model";
import { maintainerLockPath } from "@features/rule-maintenance/paths";
import { protectedRulePathsForBatch, protectedRulePathsOverlap } from "@features/rule-maintenance/protected-paths";
import { formatMaintainerStatus, tailMaintenanceLog } from "@features/rule-maintenance/reporting/status";
...
import { addActiveMaintainerRun, pruneDeadActiveRuns, readActiveMaintainerRuns, removeActiveMaintainerRun, updateActiveMaintainerRun, writeActiveMaintainerRuns } from "@features/rule-maintenance/state/active-runs";
import { enqueueMaintainerBatch, readMaintainerQueue, writeMaintainerQueue } from "@features/rule-maintenance/state/queue";
...
function batchProtectedRulePaths(cwd: string, batch: MaintainerQueueBatch): string[] {
	return batch.protectedRulePaths?.length ? batch.protectedRulePaths : protectedRulePathsForBatch(cwd, batch.changedFiles);
}

function conflictsWithActive(batchProtectedRulePaths: string[], active: MaintainerActiveRun[]): boolean {
	return active.some((run) => protectedRulePathsOverlap(batchProtectedRulePaths, run.protectedRulePaths));
}

function startScheduledBatch(cwd: string, batch: MaintainerQueueBatch, protectedRulePaths: string[], reasonPrefix: string | undefined, config: HierRulesConfig, callbacks: MaintainerCallbacks): boolean {
	...
	appendMaintainerLog(cwd, `Starting maintainer run ${run.id} (${reason}) protecting ${protectedRulePaths.join(", ")} at ${run.startedAt}.`);
...
export function drainMaintainerQueue(cwd: string, config: HierRulesConfig, callbacks: MaintainerCallbacks = {}, reasonPrefix?: string): number {
	...
	while (madeProgress && active.length < maxConcurrentRuns) {
		...
		for (let index = 0; index < pending.length && active.length < maxConcurrentRuns; index += 1) {
			...
			if (startScheduledBatch(cwd, batch, protectedRulePaths, reasonPrefix, config, callbacks)) {
				...
				pending.push({ ...batch, protectedRulePaths, queuedAt: new Date().toISOString(), reason: `retry:${batch.reason}` });
				writeMaintainerQueue(cwd, pending);
			}
			break;
		}
...
export function startMaintenance(cwd: string, changedFiles: string[], reason: string, config: HierRulesConfig, callbacks: MaintainerCallbacks = {}, metadata: MaintainerPathChangeMetadata = {}): boolean {
	...
	const renamedFiles = metadata.renamedFiles?.filter((rename) => changedFileSet.has(rename.fromPath) || changedFileSet.has(rename.toPath));
	...
	const queueSize = enqueueMaintainerBatch(cwd, uniqueChangedFiles, reason, protectedRulePaths, filteredMetadata);
	const startedCount = drainMaintainerQueue(cwd, config, callbacks);
	if (startedCount === 0) callbacks.onQueued?.(queueSize);
	return startedCount > 0;
}

export function startNextQueuedMaintenance(cwd: string, reasonPrefix: string, config: HierRulesConfig, callbacks: MaintainerCallbacks = {}): boolean {
	const startedCount = drainMaintainerQueue(cwd, config, callbacks, reasonPrefix);
	if (startedCount > 0) appendMaintainerLog(cwd, `Started ${startedCount} queued maintainer batch(es) (${reasonPrefix}) at ${new Date().toISOString()}.`);
	return startedCount > 0;
}
...
export function terminateMaintainer(cwd: string): string {
	...
	for (const run of active) {
		...
		try {
			...
			killed += 1;
		} catch { /* best-effort emergency stop */ }
	}
	if (active.length > 0) {
		writeActiveMaintainerRuns(cwd, []);
		appendMaintainerLog(cwd, `Terminated ${killed} active maintainer process(es) and cleared active runs at ${new Date().toISOString()}.`);
	...
	if (!lock?.pid) {
		...
		appendMaintainerLog(cwd, `Maintainer kill requested at ${new Date().toISOString()}; no PID found, cleared stale lock if present.`);
		return "No maintainer PID found; cleared stale lock if present.";
	}
	if (lock.pid === process.pid) {
		fs.rmSync(maintainerLockPath(cwd), { force: true });
		appendMaintainerLog(cwd, `Maintainer kill requested at ${new Date().toISOString()}; lock pointed at current Pi PID ${lock.pid}, cleared without killing.`);
	...
	try {
		...
		fs.rmSync(maintainerLockPath(cwd), { force: true });
		appendMaintainerLog(cwd, `Terminated legacy maintainer process ${lock.pid} and cleared lock at ${new Date().toISOString()}.`);
	}
