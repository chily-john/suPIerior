...
export function pruneDeadActiveRuns(cwd: string): MaintainerActiveRun[] {
  ...
  if (alive.length !== active.length) {
    ...
    for (const run of removed)
      appendMaintainerLog(
        cwd,
        `Cleared stale maintainer active run ${run.id} for dead PID ${run.pid ?? "unknown"} at ${new Date().toISOString()}.`,
      );
...
export function updateActiveMaintainerRun(
  cwd: string,
  id: string,
  updates: Partial<MaintainerActiveRun>,
) {
  writeActiveMaintainerRuns(
    cwd,
    readActiveMaintainerRuns(cwd).map((run) => (run.id === id ? { ...run, ...updates } : run)),
  );
}
...
export function removeActiveMaintainerRun(cwd: string, id: string) {
  writeActiveMaintainerRuns(
    cwd,
    readActiveMaintainerRuns(cwd).filter((run) => run.id !== id),
  );
}
