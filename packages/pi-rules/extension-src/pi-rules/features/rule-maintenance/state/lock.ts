...
export function isMaintainerLocked(cwd: string, _config: HierRulesConfig): boolean {
  ...
  appendMaintainerLog(
    cwd,
    `Cleared stale maintainer lock for dead PID ${lock?.pid ?? "unknown"} at ${new Date().toISOString()}.`,
  );
  fs.rmSync(lockPath, { force: true });
  return false;
}

export function acquireMaintainerLock(
  cwd: string,
  reason: string,
  changedFiles: string[],
  ...
  fs.writeFileSync(
    lockPath,
    JSON.stringify({ startedAt: new Date().toISOString(), reason, changedFiles }, null, 2),
  );
  return lockPath;
}
