...
export function formatInjectedContext(selection: Selection, config: HierRulesConfig): string {
	...
	if (selection.parentSummaries.length > 0) {
		parts.push("", "### Parent Rule Summaries", "Parent summaries are routing context only. Do not read the full parent rule unless the injected child rule is insufficient.");
		for (const rule of selection.parentSummaries) parts.push(`- ${rule.relativePath}: ${rule.summary || rule.title}`);
	}
	for (const rule of selection.fullRules) {
		let content = rule.content.trim();
		if (content.length > config.injection.maxFullRuleChars) content = `${content.slice(0, config.injection.maxFullRuleChars)}\n\n[Rule truncated by pi-rules extension due to size limit.]`;
		parts.push("", `### Full Rule: ${rule.relativePath}`, "", content);
	}
	if (selection.inventories.length > 0) {
		parts.push("", "### Available Inventories", "", "Inventories are not injected by default. Read them only when you need a table of contents, reuse check, or bookkeeping update.");
		for (const rule of selection.inventories) parts.push(`- ${rule.relativePath}: ${rule.summary || rule.title}`);
	}
	return parts.join("\n");
}
