...
function scoreRule(rule: RuleFile, prompt: string, pathMentions: string[]): number {
  ...
  for (const trigger of rule.triggers) {
    ...
    if (
      normalizedTrigger &&
      (text.includes(normalizedTrigger) || compactText.includes(compactTrigger))
    )
      score += 40 + normalizedTrigger.length / 10;
  }
...
function parentCandidates(rule: RuleFile, rules: RuleFile[]): RuleFile[] {
  ...
  return parents.sort(
    (a, b) => logicalRuleDir(b).split("/").length - logicalRuleDir(a).split("/").length,
  );
}
...
export function selectRules(index: RuleIndex, prompt: string, config: HierRulesConfig): Selection {
  ...
  const explicitPathMatch = index.rules.some((rule) =>
    pathMentions.some((mentioned) =>
      rule.paths.some((pattern) => pathMatchesPattern(mentioned, pattern)),
    ),
  );
  if (!hasCodeEditIntent(prompt, explicitPathMatch))
    return {
      fullRules: [],
      parentSummaries: [],
      inventories: [],
      reason: "No code-edit intent detected",
    };
  ...
  const scored = index.rules
    ...
    .sort(
      (a, b) =>
        b.score - a.score ||
        b.rule.relativePath.split("/").length - a.rule.relativePath.split("/").length,
    );
  ...
  if (fullRules.length === 0)
    return {
      ...
      parentSummaries: [],
      inventories: [],
      reason: "No matching rule metadata",
    };

  const parentSummaries = parentCandidates(fullRules[0], index.rules).slice(
    0,
    config.injection.maxParentSummaries,
  );
  const inventories = index.rules.filter(
    (rule) =>
      rule.kind === "inventory" &&
      fullRules.some((fullRule) =>
        fullRule.paths.some((fullPattern) =>
          rule.paths.some((inventoryPattern) => fullPattern === inventoryPattern),
        ),
      ),
  );
...
export function formatSelection(selection: Selection): string {
  ...
  lines.push(
    ...(selection.fullRules.length
      ? selection.fullRules.map((rule) => `- ${rule.relativePath}`)
      : ["- none"]),
  );
  lines.push("", "Parent summaries:");
  lines.push(
    ...(selection.parentSummaries.length
      ? selection.parentSummaries.map(
          (rule) => `- ${rule.relativePath}: ${rule.summary || rule.title}`,
        )
      : ["- none"]),
  );
  lines.push("", "Available inventories:");
  lines.push(
    ...(selection.inventories.length
      ? selection.inventories.map((rule) => `- ${rule.relativePath}`)
      : ["- none"]),
  );
  return lines.join("\n");
}
