...
import { asList, getSummary, getTitle, normalizeKind, parseFrontmatter } from "@domain/rule-context/discovery/frontmatter";
...
export function getRuleIndex(cwd: string, cache: { current?: RuleIndex }): RuleIndex {
	...
	const files = findMarkdownFiles(rulesDir).sort();
	const signature = buildSignature(files);
	if (cache.current && cache.current.cwd === cwd && cache.current.signature === signature) return cache.current;
