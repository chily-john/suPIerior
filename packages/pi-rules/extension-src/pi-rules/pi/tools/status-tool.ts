...
export function registerStatusTool(pi: ExtensionAPI, state: HierRulesState) {
  pi.registerTool({
    ...
    description:
      "Inspect AGENTS.md and .pi/rules discovery status. Use only when the user asks about pi-rules status or injected rules are missing/ambiguous; do not call before normal code edits.",
