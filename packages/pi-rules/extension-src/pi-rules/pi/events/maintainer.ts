...
function ruleCoveredChangedFiles(
  cwd: string,
  files: string[],
  state: HierRulesState,
): { covered: string[]; patterns: string[] } {
  const index = getRuleIndex(cwd, state.ruleCache);
  const patterns = index.rules
    .filter((rule) => rule.kind === "rules")
    .flatMap((rule) => rule.paths);
  if (patterns.length === 0) return { covered: [], patterns };
  return {
    covered: files.filter(
      (file) =>
        !file.startsWith(".pi/") &&
        patterns.some((pattern) => pathMatchesPatternScope(file, pattern)),
    ),
    patterns,
  };
}
...
export function registerMaintainerEvents(pi: ExtensionAPI, state: HierRulesState) {
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    mergeTurnFileActivity(
      state.fileActivityThisTurn,
      fileActivityFromTool(ctx.cwd, event.toolName, event.input),
    );
  });
  ...
  pi.on("agent_end", async (_event, ctx) => {
    mergeTurnFileActivity(
      state.fileActivityThisTurn,
      getGitStatusActivitySince(ctx.cwd, state.gitStatusAtTurnStart),
    );
    const detectedFiles = allActivityPaths(state.fileActivityThisTurn);
    const { covered: changedFiles, patterns } = ruleCoveredChangedFiles(
      ctx.cwd,
      detectedFiles,
      state,
    );
    ...
    const metadata = {
      deletedFiles: deletedActivity(state.fileActivityThisTurn)
        .map(({ path }) => path)
        .filter((file) => coveredSet.has(file)),
      renamedFiles: renamedActivity(state.fileActivityThisTurn)
        .filter((rename) => coveredSet.has(rename.fromPath) || coveredSet.has(rename.toPath))
    ...
    if (detectedFiles.length === 0) {
      appendMaintainerLog(
        ctx.cwd,
        `[skip ${new Date().toISOString()}] agent_end: no changed files detected.`,
      );
      return;
    }
    if (changedFiles.length === 0) {
      appendMaintainerLog(
        ctx.cwd,
        `[skip ${new Date().toISOString()}] agent_end: no rule-covered changed files. Detected: ${detectedFiles.join(", ")}. Rule patterns: ${patterns.join(", ") || "none"}.`,
      );
      return;
    }
