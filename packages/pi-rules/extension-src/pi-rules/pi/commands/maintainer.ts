...
import { formatMaintainerStatus, startMaintenance, startNextQueuedMaintenance, tailMaintenanceLog, terminateMaintainer } from "@features/rule-maintenance";

export function registerMaintainerCommands(pi: ExtensionAPI, state: HierRulesState) {
	pi.registerCommand("pi-rules:maintain", {
		description: "Manually run hidden rules maintenance instructions for .pi/rules in the background",
	...
	pi.registerCommand("pi-rules:maintainer-status", {
		...
		handler: async (_args, ctx) => createPresenter(ctx, state.config).info(formatMaintainerStatus(ctx.cwd, state.config)),
	});

	pi.registerCommand("pi-rules:maintainer-log", {
		description: "Show the tail of the background rules maintainer log",
		handler: async (_args, ctx) => createPresenter(ctx, state.config).info(tailMaintenanceLog(ctx.cwd)),
	});

	pi.registerCommand("pi-rules:maintainer-kill", {
		description: "Terminate active background rules maintainers, clear active run state, and start queued batches if possible",
